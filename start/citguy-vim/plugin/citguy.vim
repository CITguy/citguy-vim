...
def g:CITguyStatusLine(): string
  var stl: string = ''
  # Responsive 'breakpoints'
  const LG = 50
  const MD = 35

  ### See `help statusline` for available fields

  # Left-aligned Info
  stl ..= 'b%1.3n ' #* b{buffernum}
  stl ..= '%<%f '   #* {filepath} (truncates from left)
  stl ..= '%r%m'    #* {readonly?}{modified?}
  # -- Alignment Separator
  stl ..= '%='

  # -- Right-aligned info
  if winwidth(0) >= LG
    # 0x{charcode} {line %}% {line #}:{col #}
    stl ..= ' 0x%02.B %1.3p%% %1.5l:%-2.4v'
  elseif winwidth(0) >= MD
    # {line %}% {line #}:{col #}
    stl ..= ' %p%% %l:%v'
  endif
